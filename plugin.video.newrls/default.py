...
def GetTitles(section, url, startPage= '1', numOfPages= '1'): # Get Movie Titles
        print 'newrls get Movie Titles Menu %s' % url
        ...
        for page in range( start, end):
                ...
                for movieUrl, name, img in match:
                        cm  = []
                        runstring = 'XBMC.Container.Update(plugin://plugin.video.newrls/?mode=Search&query=%s)' %(name.strip())
        		cm.append(('Search on newrls', runstring))
...
def Search(query):
        url = 'http://www.google.com/search?q=site:newrls.org ' + query
        url = url.replace(' ', '+')
        print url
